...
scene_names={
    'dtu':['scan8', 'scan21', 'scan30', 'scan31','scan34', 'scan38','scan40','scan41','scan45','scan55','scan63','scan82','scan103','scan110','scan114'],
    'llff':['orchids', 'horns', 'trex', 'fern', 'flower', 'leaves', 'room', 'fortress'],
    'replica':['room0', 'room1', 'room2',  'office0', 'office1', 'office2', 'office3']
}
...
def run_training(train_module: str, train_name: str, seed: int, cudnn_benchmark: bool=True, data_root: str='',
    ...
    settings = ws_settings.Settings(data_root, debug, arg_log_dir=args.log_dir)
    ...
    if args.slurm_id != -1:
        print(f"Warning : Overwrite scene with SLURM ARRAY TASK ID.")
        args.scene=scene_names[base_dir_train_module[1]][args.slurm_id]
    ...
    # Training:  joint_pose_nerf_training/dtu/subset_3/scan82  track_wo
    ...
    if settings.track.calculate_track_only:
        return
...
def main():
    ...
    parser.add_argument('--log_dir', type=str, default='log',
                        help='root folder to save the log file')
    parser.add_argument('--slurm_id', type=int, default=-1, help='SLURM ARRAY TASK ID, use it if you want to run massive experiments on a cloud server')
    parser.add_argument('--mipnerf', type=bool, default=False,
