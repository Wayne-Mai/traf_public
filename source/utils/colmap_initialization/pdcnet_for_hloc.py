...
def get_grid_keypoints(data: Dict[str, Any], cfg: Dict[str, Any]):
    ...
    scaling_kp = cfg.scaling_kp # scale = 2
...
class KeypointExtractor:
    ...
    def get_keypoints(self, image: np.ndarray, cfg: Dict[str, Any]):
        if self.extractor_name == 'dense_grid': # replica default
...
def extract_keypoints_and_save_h5(keypoint_extractor_module: Callable[[Any], Any], 
    ...
    """Extract keypoints as a grid (to simulate sparse keypoint extraction) and 
...
def get_matches_at_keypoints_dense_grid(flow: torch.Tensor, mask: torch.Tensor, 
    ...
    hA, wA = data_source['size_original'] # 340
    hB, wB = data_target['size_original']

    Ish, Isw = data_source['size_resized'] # 336
    ...
    if cfg.estimate_at_quarter_resolution: # * default is true
        scaling = 4.0
        ...
        scaling_for_keypoints = np.float32(data_target['size_resized_padded'])[::-1] / \
                            np.float32(size_of_flow_padded)[::-1] # * default is 4*4
    else:
        scaling_for_keypoints = [1.0, 1.0]
    # flow: 1/4 size; pB is source image, where flow starts, pB+flow=pA
    pA, pB = matches_from_flow(flow, mask, scaling=1.0)
    if pA.shape[0] < cfg.min_nbr_matches: # 3454 > 500 matches 
        # not enough matches
        ...
        YA_match = YA_match * size_of_keypoints_s[0] / float(Ish) * scaling_for_keypoints[1] # * 170 / 336 * 4
        ...
        # XY matches are in 340//2 space, original/kp space,
        ...
        if confidence_map is not None: # default there is 1/8 confidence map to append
...
def retrieve_matches_at_keypoints_locations_from_pair_list(args: Dict[str, Any], cfg: Dict[str, Any], 
    ...
    if args.homo:
        ...
        args.update(inference_parameters) # * load the model according to config here
    ...
    for i, pair in pbar:
        ...
        image0_original = imageio.imread(src_fn)[:, :, :3] # 340*600
        ...
        # * now the resized image can be divided by 8
        # do the matching
        confidence_map_from_1_to_0 = None
        flow_from_1_to_0, confidence_map_from_1_to_0, mask_from_1_to_0 = network.perform_matching(
            data['source'], data['target'], cfg, segNet=segNet) # in 1/4 resolution of size_resized
        # data'source' : ['image_original', 'size_original', 'image_resized', 'size_resized', 'image_resized_padded', 'size_resized_padded', 'image_resized_padded_torch']
