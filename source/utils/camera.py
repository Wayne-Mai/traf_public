...
def get_center_and_ray(pose_w2c: torch.Tensor, H: int, W: int, 
    ...
    grid_3D = img2cam(to_hom(xy_grid),intr) # [B,HW,3], bug happen
    ...
    return center_3D,ray # * camera center and ray direction in world space (ray is not normalized ?) 
...
def get_center_and_ray_mipnerf(pose_w2c: torch.Tensor, H: int, W: int, 
                       intr: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]: # [HW,2]
    """given the intrinsic/extrinsic matrices, get the camera center and ray directions
    at all pixels of an image of size (H, W)
    ...
    ray_hw = ray.view(batch_size, H, W, 3)
    # Distance from each unit-norm direction vector to its x-axis neighbor
    dx = torch.sqrt(torch.sum((ray_hw[:, :-1, :, :] - ray_hw[:, 1:, :, :]) ** 2, -1))
    ...
    # this is equivalent to Rctow @ K-1 @ u, because here multiplied by the whole P and not just R, 
    # so need to substract the translation vector
    return center_3D,ray, radii.reshape(batch_size,-1,1) # * camera center and ray direction in world space (ray is not normalized ?) 

def get_center_and_ray_at_pixels_mipnerf(pose_w2c: torch.Tensor, pixels: torch.Tensor, 
                                 intr: torch.Tensor, H: int, W: int) -> Tuple[torch.Tensor, torch.Tensor]: # [HW,2]
    """ 
    given the intrinsic/extrinsic matrices, get the camera center and ray directions
    of a specific set of pixles
    ...
    """
    # import pdb; pdb.set_trace()
    pixels_l = pixels.long()
    center_3D,ray, radii = get_center_and_ray_mipnerf(pose_w2c, H, W, intr)
    center_3D = center_3D.reshape(-1, H, W, 3)[:,pixels_l[...,1], pixels_l[...,0]]
    ray = ray.reshape(-1, H, W, 3)[:,pixels_l[...,1], pixels_l[...,0]]
    radii = radii.reshape(-1, H, W, 1)[:,pixels_l[...,1], pixels_l[...,0]]
    return center_3D,ray, radii # * camera center and ray direction in world space (ray is not normalized ?) 
