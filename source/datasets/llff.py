...
class LLFFPerScene(base.Dataset):
    ...
    def __init__(self, args: Dict[str, Any], split: str="train", **kwargs):
        ...
        self.path = os.path.join(args.env.llff, args.scene)  # /../../horns
        self.scene = args.scene # horns
        ...
        if hasattr(self.args, 'llff_img_factor') and self.args.llff_img_factor > 1:
            imgdir_suffix = f'_{self.args.llff_img_factor}' # _8
            factor = self.args.llff_img_factor # means the resolution is 1/8
        else:
            factor = 1
        # /../horns/images_8
        self.path_image = "{}/images".format(self.path)  + imgdir_suffix
        image_fnames = [f for f in sorted(os.listdir(self.path_image)) if \
            f.endswith('JPG') or f.endswith('jpg') or f.endswith('png')]  # N, name w/o path
        ...
        self.bounds = bounds # [62,2], images, [near_1.4m, far_19m]
        ...
        if split != 'train' and self.args.val_sub is not None: 
            self.list = self.list[:self.args.val_sub] # train_sub, name w/o path

        # preload dataset
        if self.args.preload: # default false
