...
class DepthConsistencyLoss(BaseLoss):
    ...
    def compute_loss(self, opt: Dict[str, Any], data_dict: Dict[str, Any], 
        ...
        # pixels_ref: 1024,2;  depth_ref: 1024; K:3,3; pose: 4,4;
    ...
    def compute_loss_at_sampled_pose(self, data_dict: Dict[str, Any], pose_w2c_at_unseen: torch.Tensor, 
        ...
        # project pseudo gt 3D points to unseen view ==> creates pseudo-gt depth. (x,y) and (z)
        pts_in_sampled_img, pseudo_gt_depth_in_sampled_img = batch_project(pseudo_gt_3dpts_in_w, 
                                                                           ...
                                                                           return_depth=True)
        #(N, 2)
        # valid condition: xy in image, depth>0;
        ...
        # what's the depth range ? it's something like (N_train_sub, near_depth, far_depth); 0.96 or 6.24
        ...
        # this all_cumulated is similar to transparency
        ...
        if self.opt.gradually_decrease_depth_cons_loss: # false dtu
