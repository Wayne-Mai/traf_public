...
class Loss:
    ...
    def summarize_loss_w_equal_weights(self, opt: Dict[str, Any], loss_dict: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return loss_dict
    #  * note : by default it uses exponential
...
class BasePhotoandReguLoss(BaseLoss):
    ...
    def compute_loss(self, opt: Dict[str, Any], data_dict: Dict[str, Any], output_dict: Dict[str, Any], 
        ...
        if self.opt.huber_loss_for_photometric: # default true in dtu
            loss_dict.render = self.huber_loss(output_dict.rgb.reshape(nbr_rendered_images, -1, 3), image) # photo metric loss is here
        ...
        # mask loss
        if opt.loss_weight.fg_mask is not None: # default None
