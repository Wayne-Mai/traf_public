...
from source.training.core.base_depth_reg_loss import BaseDepthReguLoss
...
def define_loss(loss_type: str, opt: Dict[str, Any], nerf_net: torch.nn.Module, 
    ...
    if 'corres' in loss_type: # the correspondence is calculate here
    ...
    if 'depth_reg' in loss_type:
        loss_module.append(BaseDepthReguLoss(opt, nerf_net, train_data=train_data, device=device))
    
    if 'track' in loss_type:
        loss_module.append(TrackBasedLoss(opt, nerf_net, flow_net=flow_net, train_data=train_data, device=device))
