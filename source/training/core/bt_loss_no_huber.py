...
class TrackBasedLoss(BaseLoss, TrackUtils):
    ...
    def compute_loss_track_graph(self, opt: Dict[str, Any], data_dict: Dict[str, Any], 
        ...
        for track_graph in self.track_graph:
            ...
            temp_loss_dict,temp_stats_dict,temp_plotting_dict=self.compute_loss_at_given_tracks(opt,data_dict,
                track_graph,sampled_tracks,to_render_pixels_dict,node_record_dict)
    ...
    def refine_keypoint(self,combi_list,train_data):
        ...
        from source.training.core.track_graph import Node,Track,TrackGraph
        import pickle
        ...
        if self.opt.track.precompute_ka_for_track_loss:
            ...
            precomputed_path=os.path.join(self.opt.env.precomputed,self.opt.dataset,self.opt.scene,'trackgraph.pkl')
        ...
        for idx,match_loop in enumerate([a2b,b2a]):
            ...
            self.track_graph.append(TrackGraph(graph,track_labels,score_labels,root_labels,self.image_name_to_id,kps_new,self.device,cfg={}))
        ...
        if self.opt.track.precompute_ka_for_track_loss:
            ...
            with open(precomputed_path, 'wb') as file:
                pickle.dump(self.track_graph, file)
