...
class CorrespondencesPairRenderDepthAndGet3DPtsAndReproject(CorrespondenceBasedLoss):
    ...
    def compute_render_and_repro_loss_w_repro_thres(self, opt: Dict[str, Any], pixels_in_self_int: torch.Tensor, 
        ...
        loss = torch.norm(pts_self_repr_in_other - pixels_in_other, dim=-1, keepdim=True) # [N_rays, 1], usually sampled 1024 rays
        valid = torch.ones_like(loss).bool() # default is all true
        if opt.renderrepro_do_pixel_reprojection_check: # default false
        ...
        if opt.renderrepro_do_depth_reprojection_check: # default false
        ...
        if return_valid_mask: # default false
            return loss_corres, stats_dict, valid
        return loss_corres, stats_dict
    # * note : actual corrs loss calculation
    def compute_loss_on_image_pair(self, data_dict: Dict[str, Any], images: torch.Tensor, poses_w2c: torch.Tensor, 
        ...
        B, H, W  = images.shape[:3] # n,h,w,3
        
        
        
        
        if self.opt.track.enable_ka_for_track_loss is False:
            pixels_in_self = self.grid[mask_correct_corr]  # [N_ray, 2], absolute pixel locations of correct correspondance
            ...
            # * further demand can have a function compute_loss_on_image_pair for id
            id_in_flow_tensor, id_self, id_matching_view = self.filtered_flow_pairs[self.sampled_id_in_filtered_flow_list]
            pixels_in_self=self.pixels_map[id_in_flow_tensor][0]
            pixels_in_other=self.pixels_map[id_in_flow_tensor][1]
            
            # pixels_in_self[:, 0] = torch.clamp(pixels_in_self[:, 0], 0, W - 1)
            # pixels_in_self[:, 1] = torch.clamp(pixels_in_self[:, 1], 0, H - 1)

            # Do the same for 'pixels_in_other_rounded'
            # pixels_in_other[:, 0] = torch.clamp(pixels_in_other[:, 0], 0, W - 1)
            # pixels_in_other[:, 1] = torch.clamp(pixels_in_other[:, 1], 0, H - 1)
            
            pixels_in_self_rounded=torch.round(pixels_in_self).long()
            pixels_in_other_rounded=torch.round(pixels_in_other).long()
            
            # rescale the pixels to valid range inside the image
            # Clamp the values in 'pixels_in_self_rounded' to be within the valid range
            
            
            
            ray_in_self_int=pixels_in_self_rounded[:,1]*W+pixels_in_self_rounded[:,0]
            ray_in_other_int=pixels_in_other_rounded[:,1]*W+pixels_in_other_rounded[:,0]
            # print(ray_in_self_int.shape,ray_in_other_int.shape), H=378, W=504, 
            # min_ray,max_ray=torch.min(pixels_in_self_rounded,dim=0),torch.max(pixels_in_self_rounded,dim=0)
            # min_ray1,max_ray2=torch.min(pixels_in_other_rounded,dim=0),torch.max(pixels_in_other_rounded,dim=0)
            
            # print(H,W,min_ray,max_ray,min_ray1,max_ray2)
            
        conf_values = conf_map_self_to_other[mask_correct_corr]  # [N_ray, 1] 


        # in case there are too many values, subsamples. sometimes corr rays reach 33363 but 2048 is default ray num
        if ray_in_self_int.shape[0] > self.opt.nerf.rand_rays // 2:
            
            if ray_in_self_int.shape[0] != conf_values.shape[0]:
                print(f"Warning: {ray_in_self_int.shape[0]} and {conf_values.shape[0]} dismatches for ray shape and conf value shape ")
            sample_range=min(ray_in_self_int.shape[0],conf_values.shape[0])
            
            random_values = torch.randperm(sample_range,device=self.device)[:self.opt.nerf.rand_rays//2]
            
            # print(conf_values.shape,pixels_in_self.shape,pixels_in_other.shape,ray_in_self_int.shape,ray_in_other_int.shape)
        ...
        # force to reduce to 2048//2 = 1024 rays
        # * render pixels in current view, bug happpens in current view
        ...
        # it returns 'origins', 'viewdirs', 'rgb_samples', 'density_samples', 't', 'rgb', 'rgb_var', 'depth', 'depth_var', 'opacity', 'weights', 'all_cumulated', 'ray_idx'
        if self.opt.compute_photo_on_matches: # default false
        ...
        # reprojection loss 1: project p_i -> P_i -> P_ij -> p_ij; (p_ij - p_j)
        # reprojection loss 2: project p2 onto p1 and calculate the corresponding loss
