...
# * very important function, defined how we perform it
class CommonPoseEvaluation:
    ...
    def set_initial_poses(self, opt: Dict[str, Any]) -> Tuple[torch.Tensor, List[int], List[int]]:
        ...
        if opt.camera.optimize_relative_poses: # False by default
            ...
            if opt.camera.initial_pose == 'identity': # for llff
    ...
    def evaluate_test_time_photometric_optim(self, opt: Dict[str, Any], 
        ...
        for it in range(opt.optim.test_iter): # 100
...
class PoseAndNerfTrainerPerScene(nerf.NerfTrainerPerScene, CommonPoseEvaluation):
    ...
    def build_pose_net(self, opt: Dict[str, Any]):
        ...
        if opt.camera.pose_parametrization == 'axis_angle':
            ...
            # used in scnet, gbarf, also in sparf
    ...
    def evaluate_full(self,opt: Dict[str, Any], plot: bool=False, save_ind_files: bool=False, 
        ...
        results_dict = super().evaluate_full(opt, plot=plot, save_ind_files=save_ind_files, out_scene_dir=out_scene_dir)
    ...
    # in the base trainer, call super().evaluate_full_train/ evaluate_full, this nerf trainer from base
    @torch.no_grad()
    def evaluate_full_train(self,opt: Dict[str, Any], plot: bool=False, save_ind_files: bool=False, 
                      out_scene_dir: str='') -> Dict[str, torch.Tensor]:
        self.net.eval()
        # evaluate rotation/translation
        pose,pose_GT = self.get_all_training_poses(opt)
        ...
        # dump numbers
        # evaluate novel view synthesis
        results_dict = super().evaluate_full_train(opt, plot=plot, save_ind_files=save_ind_files, out_scene_dir=out_scene_dir)
        results_dict['rot_error'] = np.rad2deg(error.R.mean().item())
        results_dict['trans_error'] = error.t.mean().item()
