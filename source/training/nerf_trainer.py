...
class NerfTrainerPerScene(base.PerSceneTrainer):
    ...
    def load_dataset(self,opt: Dict[str, Any],eval_split="val"):
        ...
        self.train_data.prefetch_all_data() # * dataset note : here dataset.all method is init
        ...
        # dict_keys(['idx', 'rgb_path', 'scene', 'depth_range', 'image', 'intr', 'pose'])
        # image: n,3,378,504,  intr: n,3x3, pose: n,3,4
        # pixels/ray sampler
        ...
        if opt.nerf.depth.param == 'inverse': # default uses inverse
    ...
    def setup_optimizer(self,opt: Dict[str, Any]):
        ...
        if opt.nerf.fine_sampling and not opt.mipnerf:
    ...
    def train_step(self, iteration: int, data_dict: Dict[str, Any]
        ...
        rays = self.sampling_strategy(self.settings.nerf.rand_rays, # 2048 pixel numbers
        ...
        # * data_dict: dict_keys(['idx', 'rgb_path', 'scene', 'depth_range', 'image', 'intr', 'pose', 'iter'])
        ...
        # * here we count for various loss
    ...
    def make_result_dict(self, opt: Dict[str, Any], data_dict: Dict[str, Any], output_dict: Dict[str, Any], 
        ...
        if split=="train":
            ...
            stats_dict["lr"] = lr
            if hasattr(opt, 'nerf') and opt.nerf.fine_sampling and not opt.mipnerf:
    ...
    def get_all_training_poses(self,opt):
        ...
        return None, pose_GT
    # * note : main function for evaluation
    @torch.no_grad()
    def evaluate_full(self, opt: Dict[str, Any], plot: bool=False, 
        ...
        self.logger.info(message)
        return results_dict
    # * note : the novel view staff is from here
    @torch.no_grad()
    ...
    @torch.no_grad()
    def evaluate_full_train(self, opt: Dict[str, Any], plot: bool=False, 
                      save_ind_files: bool=False, out_scene_dir: str=''):
        """
        Does the actual evaluation here on the test set. Important that opt is given as variable to change
        the test time optimization input. 
        ...
        self.logger.info("Entering train dataset evaluation. For visualization purpose only")
