...
from source.models.mipnerf import MipNeRF
...
class Graph(torch.nn.Module):
    ...
    def define_renderer(self, opt: Dict[str, Any]):
        ...
        if opt.mipnerf:
            self.nerf = MipNeRF(white_bkgd=opt.nerf.setbg_opaque or opt.mask_img,
                                device=self.device,
                                
                                ).to(self.device)
        else:
            self.nerf = NeRF(opt).to(self.device)
            if opt.nerf.fine_sampling: # default false for replica, true for dtu
    ...
    def get_network_components(self):
        ret = [self.nerf]
        if self.opt.nerf.fine_sampling and not self.opt.mipnerf:
    ...
    def forward(self,opt: Dict[str, Any], data_dict: Dict[str, Any], iter: int, 
        ...
        return ret
    # * note : used function of rendering
        ...
        if ray_idx is None and pixels is None:
            ...
            ret = self.render_by_slices_w_custom_rays(opt,pose,intr=intr,pixels=pixels,ray_idx=ray_idx,mode=mode, 
                  ...
                  self.render(opt,pose,intr=intr,pixels=pixels,ray_idx=ray_idx,mode=mode, 
                              H=H, W=W, depth_range=depth_range, iter=iter) # [B,N,3],[B,N,1], render is also called by default in depth
            ret.ray_idx = ray_idx  
    ...
    def render_image_at_specific_rays(self, opt: Dict[str, Any], data_dict: Dict[str, Any], iter: int, 
        ...
        else: # * given pixels and ray_idx to do the rendering 
            ret = self.render_by_slices_w_custom_rays(opt,pose,intr=intr,pixels=pixels,ray_idx=ray_idx,mode=mode, 
                                                      H=H, W=W, depth_range=depth_range, iter=iter) if per_slice else \
                  self.render(opt,pose,intr=intr,pixels=pixels,ray_idx=ray_idx,mode=mode,  # * default calling render function
        ...
        ret.idx_img_rendered = torch.from_numpy(np.array(img_idx)).to(self.device) if img_idx is not None else \
            torch.arange(start=0, end=batch_size).to(self.device) # image 0,1,2 global image idx
        return ret
    # * always called by dtu corrs loss and photometric loss + depth loss
    def render(self, opt: Dict[str, Any], pose: torch.Tensor, H: int, W: int, intr: torch.Tensor, pixels: torch.Tensor = None, 
        ...
        Args:
            ...
            pixels, ray_idx  (torch.Tensor): if any of the two is specified, will render only at these locations. 
                             (L, N, 2) or (N, 2)  / (L, N) or (N); corrs loss: given as pixel coordinates;
            mode (str, optional): Defaults to None.
        """
        ...
        if pixels is not None:
            if opt.mipnerf:
                center, ray, radii = camera.get_center_and_ray_at_pixels_mipnerf(pose, pixels, intr=intr, H=H, W=W) 
            else:
            ...
            if opt.mipnerf:
                center, ray, radii = camera.get_center_and_ray_mipnerf(pose, H, W, intr=intr) 
            else:
            ...
            if ray_idx is not None:
                if len(ray_idx.shape) == 2 and ray_idx.shape[0] == batch_size:
                    ...
                    if opt.mipnerf:
                        radii = radii[batch_idx_flattened, ray_idx_flattened].reshape(batch_size, n_rays, 1)
                else:
                    # consider only subset of rays
                    # ray_idx is (-1)
                    center,ray = center[:,ray_idx],ray[:,ray_idx]  # [B, N_rays, 3], only fetch ray idx
                    if opt.mipnerf:
                        radii = radii[:,ray_idx]
        ...
        if opt.mipnerf:
            ...
            viewdirs = ray / ray.norm(dim=-1, keepdim=True)
            rays = edict(origins=center, directions=ray, viewdirs=viewdirs, radii=radii.reshape(-1,1), near=ones*depth_range[0], far=ones*depth_range[1])
            coarse, fine = self.nerf(rays)
            pred.update(coarse)
            fine = edict({k+'_fine': v for k, v in fine.items()})
            pred.update(fine)
            pred = edict({k:v.reshape(batch_size,n_rays,-1) for k, v in pred.items()})
            pred["all_cumulated"] = pred["all_cumulated"].squeeze(-1)
            pred["all_cumulated_fine"] = pred["all_cumulated_fine"].squeeze(-1)
            # pass
        else:
            if opt.camera.ndc: # default false for dtu
            ...
            weights = pred_coarse['weights'] # ['rgb_samples', 'density_samples', 't', 'rgb', 'rgb_var', 'depth', 'depth_var', 'opacity', 'weights', 'all_cumulated']
            # * Sampling Strategy: The coarse stage uses a uniform sampling strategy along the rays, while the fine stage uses importance sampling based on the coarse stage's output.
            ...
            if hasattr(opt.nerf, 'ratio_start_fine_sampling_at_x') and opt.nerf.ratio_start_fine_sampling_at_x is not None \
                ...
                compute_fine_sampling = False # dtu false, eval is true

            if opt.nerf.fine_sampling and compute_fine_sampling: # * default just coarse stage, surprisingly...
                with torch.no_grad():
    ...
    def render_by_slices(self, opt: Dict[str, Any], pose: torch.Tensor, H: int, W: int, 
        ...
        return ret_all
    # * note : looks like depth sampling
    def sample_depth(self, opt: Dict[str, Any], batch_size: int, n_samples: int, H: int, W: int, depth_range: List[float], \
        ...
        Args:
            ...
            n_samples (int): Number of depth samples along each ray, default 128 for llff
            H (int): img size
            W (int): img size
            depth_range (list)
        ...
        if opt.nerf.sample_stratified and mode not in ['val', 'eval', 'test']: # default sample_stratified is true
            # random samples
        ...
        # [0,1,2,...127] + noise around (0,1) -> rand_samples
        rand_samples += torch.arange(n_samples,device=self.device)[None,None,:,None].float() # the added part is [1, 1, N, 1] ==> [B,HW,N,1]
        depth_samples = rand_samples/n_samples*(depth_max-depth_min)+depth_min # [B,HW,N,1], scale according to depth range
        # almost linear
        depth_samples = dict(
        ...
        return depth_samples  # n,num_rays,num_points_per_ray,1; every element is a depth on a ray
    ...
    def render_up_to_maxdepth_at_specific_pose_and_rays\
        ...
        ret = self.render_by_slices_w_custom_rays(opt,pose,intr=intr,pixels=pixels,ray_idx=ray_idx,mode=mode, 
                ...
                self.render_to_max(opt,pose,intr=intr,pixels=pixels,ray_idx=ray_idx,mode=mode, 
                                 H=H, W=W, depth_min=depth_range[0], depth_max=depth_max, iter=iter) # [B,N,3],[B,N,1], this render is just for a valid mask
        ret.ray_idx = ray_idx  
        # ret.update(rgb_fine=rgb_fine,depth_fine=depth_fine,opacity_fine=opacity_fine) # [1, N_rays, K] or [N, N_rays, K]
        return ret
    ...
    def render_to_max(self, opt: Dict[str, Any], pose: torch.Tensor, H: int, W: int, intr: torch.Tensor, 
        ...
        if pixels is not None:
            if opt.mipnerf:
                center, ray, radii = camera.get_center_and_ray_at_pixels_mipnerf(pose, pixels, intr=intr, H=H, W=W) #  todo bug
            else:
                center, ray = camera.get_center_and_ray_at_pixels(pose, pixels, intr=intr) #  todo bug
            # [B, N_rays, 3] and [B, N_rays, 3]
        else:
            # from ray_idx
            
            if opt.mipnerf:
                center, ray, radii = camera.get_center_and_ray_mipnerf(pose, H, W, intr=intr) #  todo bug
            else:
                center,ray = camera.get_center_and_ray(pose, H, W, intr=intr) # [B,HW,3],
            while ray.isnan().any(): # TODO: weird bug, ray becomes NaN arbitrarily if batch_size>1, not deterministic reproducible
                if opt.mipnerf:
        ...
        if opt.mipnerf:
            ...
            ones = torch.ones_like(center[..., :1])
            viewdirs = ray / ray.norm(dim=-1, keepdim=True)
            rays = edict(origins=center, directions=ray, viewdirs=viewdirs, radii=radii.reshape(-1,1), near=ones*depth_min, far=depth_max.reshape(center.shape[0], 1))
            coarse, fine = self.nerf(rays)
            pred.update(coarse)
            fine = edict({k+'_fine': v for k, v in fine.items()})
            pred.update(fine)
            ...
            # can only be in metric depth, the max depth is from other image's depth, min depth
    ...
    def sample_depth_diff_max_range_per_ray(self, opt: Dict[str, Any], batch_size: int, n_samples: int, H: int, W: int, 
        ...
        rand_samples += torch.arange(n_samples,device=self.device)[None,None,:,None].float() # the added part is [1, 1, N, 1] ==> [B,HW,N,1]
        # rand_samples: 1,764,128,1; [1/128,...,1] * depth_max
        # depth_max is (B, HW)
