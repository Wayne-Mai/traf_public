...
def get_base_config():
    ...
    # loss module
    cfg.loss_type = 'photometric'                   # candidates : corrs, depth, track, separate the options by 'and'
    ...
    cfg.val_steps = 5000
    cfg.snapshot_steps = 10000                                       # save checkpoint (every N epochs), same than val step (every N epochs) ==> what is an epoch here?                                      
    # * note params: improve from 5000
    
    
    # cfg.track = edict()
    # cfg.track.enable_ka = False                                
    
    # * note : added for our track loss and keypoint adjustment
    # track optimization config
    cfg.track=edict()
    cfg.track.precompute_ka_for_track_loss=True
    cfg.track.precompute_ka_for_corrs_loss=True
    cfg.track.enable_ka_for_track_loss=False # whether enable keypoint location feature metric adjustment for pairwise correspondence
    cfg.track.enable_ka_for_corrs_loss=False # whether enable keypoint location feature metric adjustment for mv correspondence
    cfg.track.mode='mv' # or mv | root
    cfg.loss_weight.tracks=-3. # for 10^
    cfg.loss_weight.render_tracks = 0.
    cfg.track.calculate_track_only=False
...
def get_nerf_default_config_llff():
    ...
    cfg.flow_ckpt_path='checkpoints/PDCNet_megadepth.pth.tar'  # change
